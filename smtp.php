        ...
        switch ($authtype) {
            ...
            case 'XOAUTH2':
                ...
                try {
                    ...
                    throw new ExceptionMailer("SMTP authentication error", 0, $e);
                }
                /*
...
class PHPMailer
{
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new ExceptionMailer($error_message);
            }
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                throw new ExceptionMailer(self::lang('provide_address'), self::STOP_CRITICAL);
            }
            ...
            if (!$this->AllowEmpty && empty($this->Body)) {
                throw new ExceptionMailer(self::lang('empty_message'), self::STOP_CRITICAL);
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new ExceptionMailer(self::lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new ExceptionMailer(self::lang('instantiate'), self::STOP_CRITICAL);
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            throw new ExceptionMailer(self::lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if ($this->UseSMTPUTF8 && !$this->smtp->getServerExt('SMTPUTF8')) {
            throw new ExceptionMailer(self::lang('no_smtputf8'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new ExceptionMailer($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) && !$this->smtp->data($header . $body)) {
            throw new ExceptionMailer(self::lang('data_not_accepted'), self::STOP_CRITICAL);
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            throw new ExceptionMailer(self::lang('recipients_failed') . $errstr, self::STOP_CONTINUE);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (static::ENCRYPTION_STARTTLS === $secure || static::ENCRYPTION_SMTPS === $secure) {
                ...
                if (!$sslext) {
                    throw new ExceptionMailer(self::lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                }
            }
            $host = $hostinfo[2];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            ...
                            throw new ExceptionMailer($message);
                        }
                    ...
                    if (
                        ...
                        throw new ExceptionMailer(self::lang('authenticate'));
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new ExceptionMailer(self::lang('extension_missing') . 'openssl');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new ExceptionMailer(self::lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment(
        ...
        try {
            if (!static::fileIsAccessible($path)) {
                throw new ExceptionMailer(self::lang('file_access') . $path, self::STOP_CONTINUE);
            }
            ...
            if (!$this->validateEncoding($encoding)) {
                throw new ExceptionMailer(self::lang('encoding') . $encoding);
            }
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!static::fileIsAccessible($path)) {
                throw new ExceptionMailer(self::lang('file_open') . $path, self::STOP_CONTINUE);
            }
        ...
        foreach ($this->CustomHeader as $k => $pair) {
            if ($pair[0] == $name) {
                ...
                if (strpbrk($name . $value, "\r\n") !== false) {
                    if ($this->exceptions) {
                        throw new ExceptionMailer(self::lang('invalid_header'));
                    }
